...
from process.process_h5ad import *
CHUNK_SIZE = 20000
...
def data_augment_v2(adata: AnnData, fixed: bool, noise_std):
   
    # 定义增强参数，例如噪声的标准差
    noise_stddev = noise_std
    augmented_01_adata = adata.copy()  
    augmented_1_adata = adata.copy()  
    augmented_10_adata = adata.copy()  
    augmented_100_adata = adata.copy()  
    gene_expression = adata.X
    
    if fixed: 
        augmented_01_adata.X = augmented_01_adata.X + np.full(gene_expression.shape, 0.1)
        augmented_1_adata.X = augmented_1_adata.X + np.full(gene_expression.shape, 1)
        augmented_10_adata.X = augmented_10_adata.X + np.full(gene_expression.shape, 10)
        augmented_100_adata.X = augmented_100_adata.X + np.full(gene_expression.shape, 100)
    else:
        # 对每个基因的表达值引入随机噪声
        # augmented_adata.X = augmented_adata.X + np.abs(np.random.normal(0, noise_stddev, gene_expression.shape))   
        print('else')
    merge_adata = adata.concatenate([augmented_01_adata, augmented_1_adata, augmented_10_adata, augmented_100_adata], join='outer')
