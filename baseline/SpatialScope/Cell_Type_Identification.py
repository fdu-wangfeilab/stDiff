...
class SpatialScopeCTI:
    ...
    def LoadLikelihoodTable(self):
        with gzip.open(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))+'/SpatialScope/extdata/Q_mat_1_1.txt.gz','rt') as f:
            lines = f.readlines()
        with gzip.open(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))+'/SpatialScope/extdata/Q_mat_1_2.txt.gz','rt') as f:
            lines += f.readlines()
        ...
        with gzip.open(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))+'/SpatialScope/extdata/Q_mat_2_1.txt.gz','rt') as f:
            lines2 = f.readlines()
        with gzip.open(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))+'/SpatialScope/extdata/Q_mat_2_2.txt.gz','rt') as f:
            lines2 += f.readlines()
        ...
        # Q_mat_all = Q1.update(Q2)
        Q_mat_all = {**Q1, **Q2}
        # print("!!!!!!!")
        # print(Q_mat_all)

        with open(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))+'/SpatialScope/extdata/X_vals.txt') as f:
            lines_X = f.readlines()
    ...
    def CellTypeIdentification(self, nu = 10, n_neighbo = 10, hs_ST = False, UMI_min_sigma = 300):
        ...
        if self.InitProp is not None:
            ...
            with open(os.path.join(self.out_dir, 'InitProp.pickle'), 'rb') as handle:
                ...
                self.LoadLikelihoodTable()
        cell_locations = cell_locations.loc[cell_locations['spot_index'].isin(self.InitProp['results'].index.values)] # 问题在这里
        print(cell_locations)
